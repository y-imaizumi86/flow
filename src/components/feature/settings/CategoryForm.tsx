  ...
  return (
    <form onSubmit={handleSubmit} className="relative flex flex-col gap-6 p-4 pb-40">
      <div className="flex flex-col items-center gap-4 py-4">
        ...
        {isEditMode && !['その他', 'Other', 'Others'].includes(initialValues?.name ?? '') && (
          <button
            type="button"
        ...
        <input
          ...
          disabled={['その他', 'Other', 'Others'].includes(initialValues?.name ?? '')}
        ...
        <div className="space-y-3">
          ...
          <div className="no-scrollbar flex gap-3 overflow-x-auto px-1 py-2">
            {AVAILABLE_COLORS.map((color) => (
              ...
              >
                {selectedColor === color && <Check className="size-4" />}
              </button>
            ))}
          </div>
        </div>
